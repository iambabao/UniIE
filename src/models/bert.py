...
from .utils import ce_loss_with_split as ce_loss
...
class BertClassifier(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.start_dropout = nn.Dropout(config.hidden_dropout_prob)
        self.end_layer = nn.Linear(config.hidden_size, config.hidden_size)
        self.end_dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(
        ...
        start_output = self.start_dropout(self.start_layer(sequence_output))
        end_output = self.end_dropout(self.end_layer(sequence_output))
        ...
        task_logits = [self.output_layers[task_id](task_matrix[batch_id]) for batch_id, task_id in enumerate(task_id)]
        outputs = (task_logits,) + outputs

        if labels is not None:
            labels = labels.reshape(-1, max_seq_length, max_seq_length)
            loss = ce_loss(task_logits, labels, position_mask)
